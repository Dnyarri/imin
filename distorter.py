...
"""Image interpolation algorithms test shell; general displacement; function-based variant.
...
from tkinter import Button, DoubleVar, Frame, Label, Menu, Menubutton, OptionMenu, PhotoImage, Spinbox, StringVar, Tk
...
def RunFilter(event=None) -> None:
    ...
    # ↓ displacing
    start = time()
    image3D = displace(source_image3D, fx, fy, XNEW, YNEW, edge=edge, method=method)
    timing = time() - start
    ...
    # ↓ Adding filename, mode and status to window title a-la Photoshop
    sortir.title(f'{product_name}: {Path(sourcefilename).name}{color_mode_str}{"*" if is_filtered else ""}')
    info_normal = {'txt': f'{Path(sourcefilename).name}{"*" if is_filtered else ""} X={X if is_filtered else X} Y={Y if is_filtered else Y} Z={Z} maxcolors={maxcolors}', 'fg': 'grey', 'bg': 'grey90'}
    UINormal()
...
info00 = Label(frame_top, text='Displacement:', font=('helvetica', 8, 'italic'), justify='right', foreground='brown', state='disabled')
...
method_menu.grid(row=1, column=col, columnspan=3)
...
in01 = Spinbox(frame_top, from_=-1.00, to=1.00, increment=0.01, textvariable=ini_x, format='%1.2f', state='disabled', width=5, font=('helvetica', 11), validate='key', validatecommand=(validate_entry, '%S'))
...
edge_menu.grid(row=1, column=col, columnspan=3)
edge_menu.configure(indicatoron=True, font=('helvetica', 12), width=6, anchor='e', relief='groove', activebackground='#E5F1FB', state='disabled')
...
ini_y = DoubleVar(value=0.0)
in02 = Spinbox(frame_top, from_=-1.00, to=1.00, increment=0.01, textvariable=ini_y, format='%1.2f', state='disabled', width=5, font=('helvetica', 11), validate='key', validatecommand=(validate_entry, '%S'))
...
transparent_controls = (in01, in02)  # To be cut off global evens
